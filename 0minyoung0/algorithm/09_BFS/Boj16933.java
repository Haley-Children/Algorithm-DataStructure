...
public class Boj16933 {
    public static void main(String[] args) throws Exception {
        ...
        while (!q.isEmpty()) {
        	int[] cur = q.poll();
        	// cur[0], cur[1]은 x, y 좌표
        	// cur[2]는 남은 벽 부수기 횟수
        	// cur[3]은 제자리에서 기다린 횟수
        	for (int dir=0; dir<4; dir++) {
        		...
        		// 도착한 경우 -> 출력 후 리턴
        		if (nx == n - 1 && ny == m - 1) {
        			System.out.println(dis[cur[0]][cur[1]][cur[2]] + cur[3] + 1);
        			return;
        		}
        		
        		// 그냥 이동할 수 있는 경우
        		if (!isWall[nx][ny] && dis[nx][ny][cur[2]] == 0) {
        			q.offer(new int[] {nx, ny, cur[2], cur[3]});
        			dis[nx][ny][cur[2]] = dis[cur[0]][cur[1]][cur[2]] + 1;
        		}
        		
        		// 벽을 부수고 이동할 수 있는 경우
        		// 낮이면 벽 부수고 이동
        		// 밤이면 제자리에서 기다린 횟수를 증가시켜서 다시 큐에 넣음
        		else if (cur[2] > 0 && isWall[nx][ny] && dis[nx][ny][cur[2]-1] == 0) {
        			if ((dis[cur[0]][cur[1]][cur[2]] + cur[3]) % 2 == 1) {
